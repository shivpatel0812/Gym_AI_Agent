...
export default function DailyCoach({ date, revision, onAction }: {
  ...
  return <View style={styles.card} testID="daily-coach">
    ...
    {current ? <>
      ...
      <Text style={styles.body} numberOfLines={expanded ? undefined : 2}>{current.summary}</Text>
      {!expanded && current.priorities[0] ? <TouchableOpacity style={styles.priority}
        accessibilityRole="button" accessibilityLabel={`${current.priorities[0].title}. ${labels[current.priorities[0].action]}`}
        onPress={() => onAction(current.priorities[0].action)}>
        <Text style={styles.priorityTitle}>{current.priorities[0].title}</Text>
        <Text style={styles.link}>{labels[current.priorities[0].action]} →</Text>
      ...
      {expanded ? <>
        ...
        {current.priorities.slice(0, 5).map((item, index) => <TouchableOpacity key={`${item.id}-${index}`} style={styles.priority}
        ...
        <Text style={styles.meta}>Based on: {current.based_on.join(" · ")}.{current.unavailable.length ? ` Could not read: ${current.unavailable.map(s => s.replace(/_/g, " ")).join(", ")}.` : ""}</Text>
      </> : null}
      <View style={styles.row}>
        <TouchableOpacity accessibilityRole="button" accessibilityState={{ expanded }} onPress={() => setExpanded(v => !v)} style={styles.expandToggle}>
          <Text style={styles.link}>{expanded ? "Show less" : "See full briefing"}</Text>
        </TouchableOpacity>
        <Text style={styles.meta}>{current.source === "ai" ? "AI" : "Rules"} · {new Date(current.generated_at).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</Text>
      </View>
    </> : null}
  </View>;
}

...
export function loadStoredAiModel(): AiModelId {
  try {
    return normalizeAiModel(localStorage.getItem(AI_MODEL_STORAGE_KEY));
  } catch {
    return DEFAULT_AI_MODEL;
  }
}

export function persistAiModel(model: AiModelId) {
  try {

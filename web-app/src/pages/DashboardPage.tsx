...
export default function DashboardPage() {
  ...
  const [states, setStates] = useState<
    Record<string, { loaded: boolean; loading: boolean; error: boolean }>
  >({});
  const endpoints = {
    ...
    await Promise.all(
      Object.entries(endpoints)
        ...
        .map(async ([key, endpoint]) => {
          setStates((prev) => ({
            ...prev,
            [key]: { loaded: prev[key]?.loaded || false, loading: true, error: false },
          }));
          try {
            const { data } = await apiClient.get(`/api/${endpoint}?date_filter=${date}`, {
              timeout: 30000,
            });
            setStats((prev) => ({ ...prev, [key]: data.length }));
            setStates((prev) => ({
              ...prev,
              [key]: { loaded: true, loading: false, error: false },
            }));
          } catch {
            setStates((prev) => ({
              ...prev,
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1200px] mx-auto">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        ...
        >
          <MdTrendingUp size={18} className="text-[#FF6B35]" />
          Progress Hub
        </Link>
      </div>

      <DailyCoach date={today} />
        ...
        {statCards.map((stat) => {
          ...
          return (
            ...
            >
              ...
              {states[stat.key]?.error && (
                <div role="status" className="text-sm text-[#8E8E93] mt-2">
                  <p>
                    {states[stat.key].loaded
                      ? "Last loaded value; may be out of date."
                      : "Could not load."}
                  </p>
                  <button
                    onClick={() => void fetchStats(stat.key)}

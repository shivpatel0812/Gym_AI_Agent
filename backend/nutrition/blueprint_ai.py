...
def suggest_slot_fills(
    ...
    target_line = (
        f"{slot_target.get('calorie_min')}-{slot_target.get('calorie_max')} kcal, "
        f"{slot_target.get('protein_min')}g+ protein"
        if slot_target
        else "no slot target set"
    )
...
- anchor_verdicts is advisory. Use "adjust" to point at a real problem (protein too
  low for the slot target, calories well over) with one concrete fix. Use "solid"
  with advice like "worth keeping daily" when a meal already fits. Never suggest
